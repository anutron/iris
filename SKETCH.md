...
Two later verbs – `iris:set_dogfood` and `iris:ship_feature` – cover the "run a composed dev build locally" and "land a finished feature" motions. They're opt-in per developer via a `dogfood_branch` field in `.iris.local.toml` (the gitignored per-developer overlay; see "Local vs shared config" in [`README.md`](./README.md)); repos / developers that don't set it are unaffected. The canonical design lives in [`openspec/changes/add-dogfood-and-ship-verbs/design.md`](./openspec/changes/add-dogfood-and-ship-verbs/design.md); this is just the shape.
