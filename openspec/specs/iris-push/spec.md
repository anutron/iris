...
The plugin SHALL expose `iris:push` as an MCP tool accepting `task_id` (string, required) and `force_with_lease` (bool, default false). On success the verb SHALL return `{pushed: true, branch, remote_sha}`; on failure it SHALL return a structured error and leave origin's refs unchanged.

#### Scenario: Successful push of a task branch

- **WHEN** the verb is invoked for a task whose branch `argus/<task-slug>` has commits ahead of origin
...
- **WHEN** the verb is invoked with `force_with_lease=true` after a `git commit --amend` on the task branch
- **THEN** iris runs `git push origin <branch> --force-with-lease`, the push succeeds, and origin's ref advances to the amended SHA

#### Scenario: Non-fast-forward without `--force-with-lease` errors clearly

- **WHEN** origin's `<branch>` has a commit the worktree does not, and the verb is invoked with `force_with_lease=false`
...
- **WHEN** the user runs `iris push <task-id> [--force-with-lease]` from any shell on the host
