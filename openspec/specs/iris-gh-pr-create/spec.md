...
- **WHEN** the verb is invoked without `head` or `base_repo` for a task whose branch is `argus/<slug>` and has been pushed to origin
...
- **WHEN** the user runs `iris gh-pr-create <task-id> --title T [--body B] [--draft] [--head <name>] [--base-repo <owner/repo>]` from any shell on the host
